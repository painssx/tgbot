...
bot = telebot.TeleBot('6251117551:AAFt4aaaihdBJMoqdHsNvlA2VnOXyB9D3Cw')
...
def get_accept(message, address, full_text):
    ...
    type_yes = message.text
    products = database.get_pr_name_id()
    if type_yes == 'Подтвердить':
        # очистить ответ
        database.delete_product_from_cart(user_id)

        # отпрвим админу сообщение о новом заказе
        bot.send_message(-669247380, full_text.replace("Ваш", "Новый"))
        bot.delete_message(user_id, message_id)
